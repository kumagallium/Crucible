...
> **MCP サーバーの開発・実験のためのサンドボックス。**
> GitHub URL を貼るだけ。ビルド、デプロイ、実験 — すべてを一箇所で。

**Crucible** は MCP サーバー開発のためのセルフホスト型サンドボックスです。GitHub リポジトリの URL を貼るだけで — プライベートリポジトリにも対応 — 自動でビルド・デプロイし、SSE エンドポイントとして公開します。npm や Docker Hub への公開は不要。試して、壊して、やり直す — すべてを一つのダッシュボードから。
...
- **管理 UI** — 実験中のサーバーをダッシュボードで一覧管理。起動・停止・削除でサンドボックスを整理
...
- **研究チーム・組織** — メンバーに専門領域の MCP サーバーを開発してもらいたい環境に。共有サンドボックスで全員がデプロイ・実験可能
